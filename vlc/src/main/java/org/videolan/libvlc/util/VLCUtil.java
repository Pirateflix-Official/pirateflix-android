 ...
 * LibVlcUtil.java
 *****************************************************************************
 * Copyright © - VLC authors and VideoLAN
 *
...
public class VLCUtil {
    ...
    public static boolean hasCompatibleCPU(Context context) {
        ...
        try {
            ...
            line = br.readLine();
            if (line != null)
                frequency = Float.parseFloat(line) / .f; /* Convert to MHz */
        } catch (IOException ex) {
    ...
    private static boolean readHeader(RandomAccessFile in, ElfData elf) throws IOException {
        // http://www.sco.com/developers/gabi//ch4.eheader.html
    ...
    private static boolean readSection(RandomAccessFile in, ElfData elf) throws IOException {
        ...
        for (int i = 0; i < elf.e_shnum; ++i) {
            ...
            elf.app_offset = buffer.getInt(16);  /* Section file offset */
            elf.app_size = buffer.getInt(20);    /* Section size in bytes */
            return true;
        }
    ...
    private static boolean readArmAttributes(RandomAccessFile in, ElfData elf) throws IOException {
        byte[] bytes = new byte[elf.app_size];
        in.seek(elf.app_offset);
        in.readFully(bytes);
